...
static void ggml_fairy2i_lut_preprocess_lut16(int          m,
    ...
    for (int col = col_start; col < col_end; col += col_step) {
        ...
        for (int64_t g = g0; g < groups; g += gstep) {
            ...
            int xr0 = ggml_fairy2i_u8_to_s8_int(act_blocks[blk0].x_real[base_off + 0]);
            int xi0 = ggml_fairy2i_u8_to_s8_int(act_blocks[blk0].x_imag[base_off + 0]);
            int xr1 = ggml_fairy2i_u8_to_s8_int(act_blocks[blk0].x_real[base_off + 1]);
            int xi1 = ggml_fairy2i_u8_to_s8_int(act_blocks[blk0].x_imag[base_off + 1]);
...
// ggml_fairy2i_lut_preprocess_ex_lut16 and ggml_fairy2i_lut_preprocess_ex_lut_c
// are now defined in ggml-fairy2i-lut-qgemm.cpp (with AVX2 vectorized preprocess).
