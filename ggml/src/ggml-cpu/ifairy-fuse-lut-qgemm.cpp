...
static inline __m256 ggml_ifairy64_lut_load_scale8(const ggml_half * src) {
#    if defined(__F16C__)
...
static inline __m256 ggml_ifairy64_lut_madd(const __m256 & x, const __m256 & y, const __m256 & acc) {
#    if defined(__FMA__)
...
#    define GGML_IFAIRY64_LUT_ACCUMULATE(wt, suffix)                                                               \
        ...
        GGML_IFAIRY64_LUT_ACCUMULATE(wt1, 1);
#    undef GGML_IFAIRY64_LUT_ACCUMULATE
    }
}

static inline void ggml_ifairy64_lut_apply_pair_component(const ifairy64_lut_wtile_16 * wt0,
                                                          const ifairy64_lut_wtile_16 * wt1,
    ...
    for (int half = 0; half < 2; ++half) {
        ...
        acc = ggml_ifairy64_lut_madd(_mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(s0_a)),
                                     _mm256_mul_ps(v_a, ggml_ifairy64_lut_load_scale8(scale_a0 + offset)), acc);
        acc = ggml_ifairy64_lut_madd(_mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(s0_b)),
                                     _mm256_mul_ps(v_b, ggml_ifairy64_lut_load_scale8(scale_b0 + offset)), acc);
        acc = ggml_ifairy64_lut_madd(_mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(s1_a)),
                                     _mm256_mul_ps(v_a, ggml_ifairy64_lut_load_scale8(scale_a1 + offset)), acc);
        acc = ggml_ifairy64_lut_madd(_mm256_cvtepi32_ps(_mm256_cvtepi16_epi32(s1_b)),
                                     _mm256_mul_ps(v_b, ggml_ifairy64_lut_load_scale8(scale_b1 + offset)), acc);
    }
}

static inline void ggml_ifairy64_lut_accumulate_pair(const ifairy64_lut_wtile_16 * wt0,
                                                     const ifairy64_lut_wtile_16 * wt1,
                                                     const int8_t *                lut_blk,
                                                     const __m256 &                v_lr,
                                                     const __m256 &                v_li,
...
#    if defined(_MSC_VER)
#        define GGML_IFAIRY_NOINLINE __declspec(noinline)
#    else
#        define GGML_IFAIRY_NOINLINE __attribute__((__noinline__))
#    endif

static GGML_IFAIRY_NOINLINE void ggml_ifairy64_lut_qgemm_pair_avx2(int          m,
                                                                   int          k,
                                                                   int          n,
    ...
    for (int col = 0; col < n; ++col) {
        ...
        for (int tile = 0; tile < tiles; ++tile) {
            ...
            for (int64_t blk = 0; blk < blocks; ++blk) {
                ...
                const int8_t * lut_blk = lut_col + (size_t) blk * (size_t) groups_per_block * k_ifairy_lut_group_bytes;
                const __m256 v_lr = _mm256_set1_ps(scales[blk * 2 + 0]);
                const __m256 v_li = _mm256_xor_ps(_mm256_set1_ps(scales[blk * 2 + 1]), imag_sign);
                ggml_ifairy64_lut_accumulate_pair(wt0, wt1, lut_blk, v_lr, v_li, acc_r_lo, acc_r_hi, acc_i_lo,
                                                  acc_i_hi);
            }
            ggml_ifairy64_lut_store_pair(tile, m, dst_col, dst_row_stride, pack_bf16, add, acc_r_lo, acc_r_hi,
                                         acc_i_lo, acc_i_hi);
        }
    }
}

#    undef GGML_IFAIRY_NOINLINE
#endif
...
#if defined(__AVX2__)
    ggml_ifairy64_lut_qgemm_pair_avx2(m, k, n, packed_wtiles0, packed_wtiles1, lut, lut_scales, dst, dst_col_stride,
                                      dst_row_stride, pack_bf16, /*negate_imag_scale*/ false, /*add*/ false);
#else
    ggml_ifairy64_lut_qgemm_pair_lut16(m, k, n, packed_wtiles0, packed_wtiles1, lut, lut_scales, dst, dst_col_stride,
...
bool ggml_ifairy64_lut_qgemm_four_cpu(int          m,
    ...
    ggml_ifairy64_lut_qgemm_pair_avx2(m, k, n, packed_u0, packed_u1, lut, lut_scales, dst, dst_col_stride,
                                      dst_row_stride, pack_bf16, /*negate_imag_scale*/ true, /*add*/ false);
    ggml_ifairy64_lut_qgemm_pair_avx2(m, k, n, packed_w0, packed_w1, lut, lut_scales, dst, dst_col_stride,
