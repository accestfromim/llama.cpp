...
static __global__ void ifairy_split_kernel(
    ...
    const int64_t i0 = 1 * ((int64_t)blockDim.y * blockIdx.y + threadIdx.y);

    if (i0 >= ne0) return;
    //static_assert(sizeof(nv_bfloat16) == 2);
    ...
    const int64_t src_offset = i3 * nb03 + i2 * nb02 + i1 * nb01 + i0 * sizeof(float);
    const int64_t dst_offset = i3 * nb3  + i2 * nb2  + i1 * nb1;

    // Pointers
    const nv_bfloat16 * src_ptr = (const nv_bfloat16 *)(src0 + src_offset);
    float * dst_ptr_r = (float *)(dst + dst_offset + (i0) * sizeof(float));
    float * dst_ptr_i = (float *)(dst + dst_offset + (ne0 + i0) * sizeof(float));
...
static __global__ void ifairy_merge_kernel(
    ...
    const int64_t i0 = ((int64_t)blockDim.y * blockIdx.y + threadIdx.y);
    ...
    const int64_t dst_offset = i3 * nb3  + i2 * nb2  + i1 * nb1 + i0 * sizeof(float);

    // Pointers
    const float * src_ptr_r = (const float *)(src0 + src_offset + (i0) * sizeof(float));
    const float * src_ptr_i = (const float *)(src0 + src_offset + (ne0 + i0) * sizeof(float));
...
void ggml_cuda_op_ifairy_split(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const int n_blocks_y = (ne0 + CUDA_IFAIRY_BLOCK_SIZE - 1) / (CUDA_IFAIRY_BLOCK_SIZE);
    ...
    //printf("ggml_cuda_op_ifairy_split done\n");
}
...
void ggml_cuda_op_ifairy_merge(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    //printf("ggml_cuda_op_ifairy_merge done\n");
}
...
void ggml_cuda_op_ifairy_rmsnorm(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    //printf("ggml_cuda_op_ifairy_rmsnorm done\n");
}
...
static __global__ void ifairy_rope_kernel(
        ...
        const int ne0, const int ne1, const int ne2, const int s1, const int s2, const int s3,
    ...
    const int ix = i3 * s3 + i2 * s2 + i1 * s1 + i0_complex;
...
void ggml_cuda_op_ifairy_rope(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const size_t s03 = src0->nb[3] / sizeof(uint32_t);
    ...
    ifairy_rope_kernel<true><<<block_nums, block_dims, 0, stream>>>(
        ...
        (float *) dst->data,
        ne00, ne01, ne02, s01, s02, s03,
        n_dims, pos,
        freq_scale, freq_base
    );
    //printf("ggml_cuda_op_ifairy_rope done\n");
