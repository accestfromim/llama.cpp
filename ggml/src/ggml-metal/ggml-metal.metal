...
kernel void kernel_fairy2i_bundle_w1_half_mma32x16_k16(
        ...
        threadgroup half * coeff_imag_from_real                 [[threadgroup(2)]],
        threadgroup half * coeff_imag_from_imag                 [[threadgroup(3)]],
        threadgroup half * act_real_tile                        [[threadgroup(4)]],
        threadgroup half * act_imag_tile                        [[threadgroup(5)]],
        threadgroup float * out_tile                            [[threadgroup(6)]],
        uint2 tgpig                                             [[threadgroup_position_in_grid]],
        uint tiitg                                              [[thread_index_in_threadgroup]],
        uint sgitg                                              [[simdgroup_index_in_threadgroup]]) {
    constexpr int row_tile = 32;
    ...
    const int row_base = (int) tgpig.x * row_tile;
    const int col_base = (int) tgpig.y * 8;
    const int blocks = args.k / QK_FAIRY2I_TILE64;
    ...
    simdgroup_half8x8 a_ir;
    simdgroup_half8x8 a_ii;
    simdgroup_half8x8 b_r;
    simdgroup_half8x8 b_i;
    simdgroup_float8x8 c_r = make_filled_simdgroup_matrix<float, 8>(0.0f);
    simdgroup_float8x8 c_i = make_filled_simdgroup_matrix<float, 8>(0.0f);
    ...
    for (int wb = 0; wb < blocks; ++wb) {
        ...
        for (int k_chunk = 0; k_chunk < QK_FAIRY2I_TILE64; k_chunk += 16) {
            ...
            for (uint idx = tiitg; idx < 8 * 16; idx += n_threads) {
                const int col_local = (int) idx >> 4;
                const int k_local = (int) idx & 15;
                const int col_lane = col_local & 7;
                const int col = col_base + col_local;
                ...
                const int act_tile_idx = k_local * 8 + col_lane;
                act_real_tile[act_tile_idx] = xh.x;
                act_imag_tile[act_tile_idx] = xh.y;
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
            ...
            for (int ik = 0; ik < 2; ++ik) {
                ...
                simdgroup_load(a_ir, coeff_imag_from_real + coeff_base + ik * 8, 16);
                simdgroup_load(a_ii, coeff_imag_from_imag + coeff_base + ik * 8, 16);
                simdgroup_load(b_r, act_real_tile + ik * 64);
                simdgroup_load(b_i, act_imag_tile + ik * 64);

                simdgroup_barrier(mem_flags::mem_none);
                simdgroup_multiply_accumulate(c_r, a_rr, b_r, c_r);
                simdgroup_multiply_accumulate(c_r, a_ri, b_i, c_r);
                simdgroup_multiply_accumulate(c_i, a_ir, b_r, c_i);
                simdgroup_multiply_accumulate(c_i, a_ii, b_i, c_i);
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
    }

    const int simdgroup_out_base = (int) sgitg * 128;
    simdgroup_store(c_r, out_tile + simdgroup_out_base, 8);
    simdgroup_store(c_i, out_tile + simdgroup_out_base + 64, 8);
    threadgroup_barrier(mem_flags::mem_threadgroup);

    for (uint idx = tiitg; idx < row_tile * 8; idx += n_threads) {
        const int row_lane = (int) idx / 8;
        const int col_local = (int) idx & 7;
        const int col_lane = col_local & 7;
        const int row = row_base + row_lane;
        const int col = col_base + col_local;
        if (row < args.m && col < args.act_rows) {
            ...
            const int row_group = row_lane >> 3;
            const int row_in_group = row_lane & 7;
            const int out_base = row_group * 128;
            float out_real = out_tile[out_base + row_in_group * 8 + col_lane];
            float out_imag = out_tile[out_base + 64 + row_in_group * 8 + col_lane];
