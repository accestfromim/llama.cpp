...
template<int k_tile>
kernel void kernel_fairy2i_bundle_w1_half_mma32x16_k(
        constant ggml_metal_kargs_fairy2i_wide_linear_w2 & args [[buffer(0)]],
        device const uchar * codes                              [[buffer(1)]],
        device const half * scales                              [[buffer(2)]],
        device const half * act_h                               [[buffer(3)]],
        ...
        uint tiitg                                              [[thread_index_in_threadgroup]],
        uint sgitg                                              [[simdgroup_index_in_threadgroup]]) {
    static_assert(k_tile == 8 || k_tile == 32, "unsupported Fairy2i W1 K tile");

    constexpr int row_tile = 32;
    constexpr int n_threads = 128;
    constexpr int q4_per_chunk = k_tile / 4;
    ...
    for (int wb = 0; wb < blocks; ++wb) {
        ...
        for (int k_chunk = 0; k_chunk < QK_FAIRY2I_TILE64; k_chunk += k_tile) {
            for (uint coeff_item = tiitg; coeff_item < row_tile * q4_per_chunk; coeff_item += n_threads) {
                const int coeff_row = (int) coeff_item / q4_per_chunk;
                const int q4_local = (int) coeff_item % q4_per_chunk;
                const int row_in_m64 = (row_base + coeff_row) & 63;
                const int m16 = row_in_m64 >> 4;
                const int row_lane = row_in_m64 & 15;
                const int q4 = (k_chunk >> 2) + q4_local;
            ...
            for (int ik = 0; ik < k_tile / 8; ++ik) {
                simdgroup_load(a_rr, coeff_real_from_real + coeff_base + ik * 8, k_tile);
                simdgroup_load(a_ri, coeff_real_from_imag + coeff_base + ik * 8, k_tile);
                simdgroup_load(a_ir, coeff_imag_from_real + coeff_base + ik * 8, k_tile);
                simdgroup_load(a_ii, coeff_imag_from_imag + coeff_base + ik * 8, k_tile);
                simdgroup_load(b_r0, act_real_tile0 + ik * 64);
                simdgroup_load(b_i0, act_imag_tile0 + ik * 64);
                simdgroup_load(b_r1, act_real_tile1 + ik * 64);
                simdgroup_load(b_i1, act_imag_tile1 + ik * 64);
...
typedef decltype(kernel_fairy2i_bundle_w1_half_mma32x16_k<8>) fairy2i_bundle_w1_half_mma32x16_k_t;

template [[host_name("kernel_fairy2i_bundle_w1_half_mma32x16_k8")]]
kernel fairy2i_bundle_w1_half_mma32x16_k_t kernel_fairy2i_bundle_w1_half_mma32x16_k<8>;
template [[host_name("kernel_fairy2i_bundle_w1_half_mma32x16_k32")]]
kernel fairy2i_bundle_w1_half_mma32x16_k_t kernel_fairy2i_bundle_w1_half_mma32x16_k<32>;
