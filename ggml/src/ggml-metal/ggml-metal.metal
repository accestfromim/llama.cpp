...
// M32K256 pair2 stores the two adjacent K128 tiles for one SIMD lane in a
// single uchar4.  Bytes xy are the first K128 tile and zw are the second; each
// byte retains the v1 split8 low/high-nibble contract.
static inline int row4_weight_at_pair2(
        device const uchar * codes,
        int k_pairs,
        int output,
        int k) {
    const int output_block = output >> 5;
    const int output_lane  = output & 31;
    const int group        = output_lane >> 2;
    const int row          = output_lane & 3;
    const int k_pair       = k >> 8;
    const int k_in_pair    = k & 255;
    const int k_half       = k_in_pair >> 7;
    const int k_in_tile    = k_in_pair & 127;
    const int split        = k_in_tile >> 4;
    const int j            = k_in_tile & 7;
    const int lane         = split * 4 + (j >> 1);
    const int byte         = 2 * k_half + (j & 1);
    const ulong offset =
        (((((ulong) output_block * (ulong) k_pairs + (ulong) k_pair) * 8UL + (ulong) group) * 32UL +
          (ulong) lane) * 4UL) + (ulong) byte;
    const uchar packed = codes[offset];
    const uint code = (k_in_tile & 8) == 0 ? (uint) (packed & 0x0fU) : (uint) (packed >> 4);
...
static inline uchar row4_load_prefill_byte(
    ...
    if (args.layout == 1) {
        ...
        return codes[code_offset];
    }

    const int k_pairs      = args.k / 256;
    const uint output_block = output_tile >> 1;
    const uint global_group = (output_tile & 1U) * 4U + group;
    const int k_pair        = k_base >> 8;
    const int k_half        = (k_base >> 7) & 1;
    const int split_base    = (k_base & 127) >> 4;
    const int split         = split_base + (int) local_split;
    const int lane          = split * 4 + (int) (j >> 1);
    const int byte          = 2 * k_half + (int) (j & 1U);
    const ulong code_offset =
        (((((ulong) output_block * (ulong) k_pairs + (ulong) k_pair) * 8UL + (ulong) global_group) * 32UL +
          (ulong) lane) * 4UL) + (ulong) byte;
    return codes[code_offset];
}

static inline uchar4 row4_load_prefill_packed4(
        constant ggml_metal_kargs_row_quant_linear & args,
        device const uchar * codes,
        uint output_tile,
        int k_base,
    ...
    if (args.layout == 1) {
        ...
        return *((device const uchar4 *) (codes + code_offset));
    }
    ...
    const int split_base    = (k_base & 127) >> 4;
    const int split         = split_base + (int) local_split;
    const int lane0         = split * 4 + (int) jblock * 2;
    const ulong code_offset =
        ((((ulong) output_block * (ulong) k_pairs + (ulong) k_pair) * 8UL + (ulong) global_group) * 32UL +
         (ulong) lane0) * 4UL;
    const uchar4 pair0 = *((device const uchar4 *) (codes + code_offset));
    const uchar4 pair1 = *((device const uchar4 *) (codes + code_offset + 4UL));
    return k_half == 0 ? uchar4(pair0.x, pair0.y, pair1.x, pair1.y) :
                         uchar4(pair0.z, pair0.w, pair1.z, pair1.w);
}
...
static inline void row4_linear_reduced(
    ...
    for (int k = (int) dot_lane; k < args.k; k += (int) lanes_per_row) {
        const int weight = args.layout == 1 ? row4_weight_at(codes, k_tiles, output, k) :
                                              row4_weight_at_pair2(codes, args.k / 256, output, k);
        acc += weight * (int) act_q[act_base + (ulong) k];
    }
    partials[tid] = acc;
    threadgroup_barrier(mem_flags::mem_threadgroup);
...
static inline void row4_stage_prefill_m64n32_simd_localw(
    ...
    if (simd_lane < 16U) {
        ...
        const uchar4 packed =
            row4_load_prefill_packed4(args, codes, output_tile, k_base, group, local_split, jblock);
...
static inline void row4_decode_pair2_lut16_accumulate_dual_o4_lookahead(
    ...
    constexpr int bytes_per_k_pair   = groups_per_block * lanes_per_group * bytes_per_record;
    ...
    for (; kp + 1 < k_pairs; kp += 2) {
        ...
        const char2 current_activation0_high = *((threadgroup const char2 *) (act_ptr + 8));
        const char2 current_activation1_low  = *((threadgroup const char2 *) (act_ptr + 128));
        const char2 current_activation1_high = *((threadgroup const char2 *) (act_ptr + 136));

        device const uchar * next_code0_ptr = code0_ptr + bytes_per_k_pair;
        device const uchar * next_code1_ptr = code1_ptr + bytes_per_k_pair;
        threadgroup const char * next_act_ptr = act_ptr + k_pair;
        const uchar4 next_packed0 = *((device const uchar4 *) next_code0_ptr);
        const uchar4 next_packed1 = *((device const uchar4 *) next_code1_ptr);
        const char2 next_activation0_low  = *((threadgroup const char2 *) next_act_ptr);
        const char2 next_activation0_high = *((threadgroup const char2 *) (next_act_ptr + 8));
...
static inline void row4_w1a8_prefill_impl(
    ...
    for (int k_base = 0; k_base < args.k; k_base += k_tile) {
        for (uint packed_index = tid; packed_index < 64U; packed_index += n_threads) {
            ...
            const uchar packed =
                row4_load_prefill_byte(args, codes, tgpig.x, k_base, group, local_split, j);
...
constant float turbo_centroids_3bit[8]  = { -0.190207f, -0.118786f, -0.066822f, -0.021663f,
...
template <typename type4x4> void dequantize_turbo2_0(const device block_turbo2_0 * xb, short il, thread type4x4 & reg) {
    ...
    for (int i = 0; i < 16; ++i) {
        const uint8_t q      = xb->qs[(base + i) / 4];
        values[i / 4][i % 4] = norm * turbo_centroids_2bit[(q >> (2 * ((base + i) % 4))) & 0x3];
    }
    reg = (type4x4) values;
}

template <typename type4x4> void dequantize_turbo3_0(const device block_turbo3_0 * xb, short il, thread type4x4 & reg) {
    const float norm = float(xb->norm);
    float4x4    values;
    const int   base = 16 * il;
    for (int i = 0; i < 16; ++i) {
        const int     j      = base + i;
        const uint8_t low    = (xb->qs[j / 4] >> (2 * (j % 4))) & 0x3;
...
template [[host_name("kernel_flash_attn_ext_kturbo2_vturbo2_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo2_vturbo3_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo2_0,
                          8,
                          dequantize_turbo2_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo2_vturbo4_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo2_0,
                          8,
                          dequantize_turbo2_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo3_vturbo2_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo3_0,
                          8,
                          dequantize_turbo3_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo3_vturbo3_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo3_0,
                          8,
                          dequantize_turbo3_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo3_vturbo4_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo3_0,
                          8,
                          dequantize_turbo3_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo4_vturbo2_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo4_0,
                          8,
                          dequantize_turbo4_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo4_vturbo3_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo4_0,
                          8,
                          dequantize_turbo4_0,
                          ...
                          128,
                          128>;
template [[host_name("kernel_flash_attn_ext_kturbo4_vturbo4_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES,
                          block_turbo4_0,
                          8,
                          dequantize_turbo4_0,
...
template [[host_name("kernel_flash_attn_ext_kturbo3_vq8_0_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES, block_turbo3_0, 8, dequantize_turbo3_0, block_q8_0, 2, dequantize_q8_0, 128, 128>;
template [[host_name("kernel_flash_attn_ext_kturbo4_vq8_0_dk128_dv128")]] kernel flash_attn_ext_t
    kernel_flash_attn_ext<FA_TYPES, block_turbo4_0, 8, dequantize_turbo4_0, block_q8_0, 2, dequantize_q8_0, 128, 128>;
...
template [[host_name("kernel_cpy_row4_codes_pair2_row4_codes_pair2")]]
kernel kernel_cpy_t kernel_cpy<uchar, uchar>;
#if defined(GGML_METAL_HAS_BF16)
...
kernel void kernel_turbo_wht(constant ggml_metal_kargs_turbo_wht & args,
...
static uint8_t turbo_index_3bit(float x) {
    if (x < -0.154496f) {
        return 0;
    }
    if (x < -0.092804f) {
...
static uint8_t turbo_index_4bit(float x) {
    if (x < -0.212203f) {
        return 0;
    }
    if (x < -0.162947f) {
...
static void turbo_pack(const thread float * x, float norm, device block_turbo2_0 & dst) {
    float recon_norm2 = 0.0f;
    for (int i = 0; i < 32; ++i) {
        dst.qs[i] = 0;
    }
    for (int i = 0; i < 128; ++i) {
        const uint8_t q = turbo_index_2bit(x[i]);
        dst.qs[i / 4] |= q << (2 * (i % 4));
        recon_norm2 += turbo_centroids_2bit[q] * turbo_centroids_2bit[q];
    }
    const float recon_norm = sqrt(recon_norm2);
    dst.norm               = half(recon_norm > 1e-10f ? norm / recon_norm : norm);
}

static void turbo_pack(const thread float * x, float norm, device block_turbo3_0 & dst) {
    float recon_norm2 = 0.0f;
    for (int i = 0; i < 32; ++i) {
        dst.qs[i] = 0;
    }
    ...
    for (int i = 0; i < 128; ++i) {
        const uint8_t q = turbo_index_3bit(x[i]);
        dst.qs[i / 4] |= (q & 0x3) << (2 * (i % 4));
        dst.signs[i / 8] |= ((q >> 2) & 0x1) << (i % 8);
        recon_norm2 += turbo_centroids_3bit[q] * turbo_centroids_3bit[q];
    }
    const float recon_norm = sqrt(recon_norm2);
    dst.norm               = half(recon_norm > 1e-10f ? norm / recon_norm : norm);
}

static void turbo_pack(const thread float * x, float norm, device block_turbo4_0 & dst) {
    float recon_norm2 = 0.0f;
    for (int i = 0; i < 64; ++i) {
        dst.qs[i] = 0;
    }
    for (int i = 0; i < 128; ++i) {
        const uint8_t q = turbo_index_4bit(x[i]);
        dst.qs[i / 2] |= q << (4 * (i % 2));
        recon_norm2 += turbo_centroids_4bit[q] * turbo_centroids_4bit[q];
    }
    const float recon_norm = sqrt(recon_norm2);
    dst.norm               = half(recon_norm > 1e-10f ? norm / recon_norm : norm);
}

template <typename TI, typename block_q>
kernel void kernel_set_rows_turbo(constant ggml_metal_kargs_set_rows & args,
    ...
    device block_q * dst_row =
        (device block_q *) ((device char *) dst + int64_t(row) * args.nb1 + i02 * args.nb2 + i03 * args.nb3);
    const device float * src_row =
        (const device float *) ((const device char *) src0 + i01 * args.nb01 + i02 * args.nb02 + i03 * args.nb03);

    for (int block = tiitg % tptg.x; block < args.nk0; block += tptg.x) {
        thread float x[128];
        float        norm2 = 0.0f;
        for (int i = 0; i < 128; ++i) {
            const float value = src_row[128 * block + i];
            x[i]              = value;
            norm2 += value * value;
        }
...
template
    [[host_name("kernel_set_rows_turbo2_i32")]] kernel set_rows_turbo2_t kernel_set_rows_turbo<int32_t, block_turbo2_0>;
template
    [[host_name("kernel_set_rows_turbo3_i64")]] kernel set_rows_turbo3_t kernel_set_rows_turbo<int64_t, block_turbo3_0>;
template
