    ...
    for (int wb = 0; wb < blocks; ++wb) {
        ...
        for (int k_chunk = 0; k_chunk < QK_FAIRY2I_TILE64; k_chunk += 16) {
            ...
            for (int ik = 0; ik < 2; ++ik) {
                ...
                simdgroup_multiply_accumulate(c_r0, a_rr, b_r, c_r0);
                simdgroup_multiply_accumulate(c_r0, a_ri, b_i, c_r0);
                simdgroup_multiply_accumulate(c_i0, a_ir, b_r, c_i0);
                simdgroup_multiply_accumulate(c_i0, a_ii, b_i, c_i0);

                simdgroup_load(b_r, act_real_tile1 + ik * 64);
                simdgroup_load(b_i, act_imag_tile1 + ik * 64);
                simdgroup_multiply_accumulate(c_r1, a_rr, b_r, c_r1);
                simdgroup_multiply_accumulate(c_r1, a_ri, b_i, c_r1);
                simdgroup_multiply_accumulate(c_i1, a_ir, b_r, c_i1);
                simdgroup_multiply_accumulate(c_i1, a_ii, b_i, c_i1);
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
        }
    }
