...
kernel void kernel_fairy2i_bundle_w1_bf16_tile8x1_sg4_full_nobias_fc(
    ...
    const int row_local_base = simd_group * rows_per_simdgroup;
    const int block_slot = simd_lane >> 4;
    const int q4 = simd_lane & 15;
    const int blocks = FC_fairy2i_bundle_w1_decode_blocks;
    const int m16 = (row_base & 63) >> 4;
    const int row_lane_base = row_base & 15;

    float acc_real[rows_per_simdgroup] = { 0.0f };
    float acc_imag[rows_per_simdgroup] = { 0.0f };
    ...
    for (int wb_base = 0; wb_base < blocks; wb_base += block_slots) {
        ...
        if (wb < blocks) {
            ...
            FOR_UNROLL (int tr = 0; tr < rows_per_simdgroup; ++tr) {
                const int row_offset = row_local_base + tr;
                const uint2 packed = uint2(
                    (uint) codes[code_base + row_offset],
                    (uint) codes[code_base + 16 + row_offset]);
                fairy2i_accumulate_tile_weight2_decode_f32_reg(
                    packed, wr, wi, act_real, act_imag, tr, acc_real, acc_imag);
            }
        }
    ...
    FOR_UNROLL (int tr = 0; tr < rows_per_simdgroup; ++tr) {
        const float real = simd_sum(acc_real[tr]);
        const float imag = simd_sum(acc_imag[tr]);
        if (simd_lane == 0) {
            const int row = row_base + row_local_base + tr;
            *((device uint *) (dst + (ulong) row * FC_fairy2i_bundle_w1_decode_dst_nb0)) =
                fairy2i_pack_bf16_pair(real, imag);
        }
    }
