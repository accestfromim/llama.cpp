...
kernel void kernel_fairy2i_bundle_w1_half_mma_mn_k16(
        constant ggml_metal_kargs_fairy2i_wide_linear_w2 & args [[buffer(0)]],
        device const uchar * codes                              [[buffer(1)]],
        device const half * scales                              [[buffer(2)]],
        device const half * act_h                               [[buffer(3)]],
        ...
        threadgroup half * coeff_imag_from_real                 [[threadgroup(2)]],
        threadgroup half * coeff_imag_from_imag                 [[threadgroup(3)]],
        threadgroup half * act_real                             [[threadgroup(4)]],
        threadgroup half * act_imag                             [[threadgroup(5)]],
    ...
    constexpr int out_elements = row_tile * col_tile;
    ...
    for (int wb = 0; wb < blocks; ++wb) {
        ...
        for (int k_chunk = 0; k_chunk < QK_FAIRY2I_TILE64; k_chunk += 16) {
            for (uint coeff_item = tiitg; coeff_item < row_tile * 4; coeff_item += n_threads) {
                const int coeff_row = (int) coeff_item >> 2;
                const int q4_local = (int) coeff_item & 3;
                const int row_in_m64 = (row_base + coeff_row) & 63;
                const int m16 = row_in_m64 >> 4;
                const int row_lane = row_in_m64 & 15;
                const int q4 = (k_chunk >> 2) + q4_local;
            ...
            for (uint idx = tiitg; idx < col_tile * 16; idx += n_threads) {
                const int col_local = (int) idx >> 4;
                const int k_local = (int) idx & 15;
                const int col = col_base + col_local;
                ...
                const int act_index = k_local * col_tile + col_local;
                act_real[act_index] = xh.x;
                act_imag[act_index] = xh.y;
            }
            threadgroup_barrier(mem_flags::mem_threadgroup);
    ...
    for (uint idx = tiitg; idx < out_elements; idx += n_threads) {
        const int row_lane = (int) idx / col_tile;
        ...
        if (row < args.m && col < args.act_rows) {
            ...
            const int i2 = (col / args.x_ne1) % args.x_ne2;
            const int i3 = col / (args.x_ne1 * args.x_ne2);
            float out_real = out_tile[idx];
            float out_imag = out_tile[out_elements + idx];
