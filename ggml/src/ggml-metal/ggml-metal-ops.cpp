...
static ggml_metal_buffer_id ggml_metal_get_buffer_id_for_size(const ggml_tensor * t, size_t size) {
    if (!t) {
        return { nullptr, 0 };
    }
    ...
    return ggml_metal_buffer_get_id_for_size(ctx, t, size);
}
...
static int ggml_metal_op_encode_impl(ggml_metal_op_t ctx, int idx) {
    ...
    switch (node->op) {
        ...
        case GGML_OP_TURBO_K_MEAN_CENTER:
            {
                n_fuse = ggml_metal_op_turbo_k_mean_center(ctx, idx);
            }
            break;
...
static bool ggml_metal_turbo2m4_force_generic() {
    static const bool force = [] {
        const char * value = getenv("GGML_METAL_TURBO2M4_FORCE_GENERIC");
...
static bool ggml_metal_get_turbo4_fattn_fusion(ggml_metal_op_t ctx, int idx, ggml_metal_turbo4_fattn_fusion * fusion) {
    if (!ctx->use_fusion || idx + 3 >= ctx->idx_end || ggml_metal_turbo2m4_force_generic()) {
        return false;
    }
    ...
    int q_idx = idx - 1;
    while (q_idx >= 0 && ggml_graph_node(ctx->gf, q_idx) != q_wht) {
        --q_idx;
    }
    if (q_idx < 0 || !ggml_node_has_n_uses(ctx->gf, q_idx, 1)) {
        return false;
    }
...
struct ggml_metal_row4_m5_tensorops_config {
    const char * pipeline_name;
    const char * path_detail;
    int          row_tile;
    int          output_tile;
    int          n_simdgroups;
...
static ggml_metal_row4_m5_tensorops_config ggml_metal_row4_m5_tensorops_select(bool    available,
    ...
    if (!available || has_residual || (layout != 1 && layout != 2) || act_rows <= 0 || m <= 0 || k <= 0 ||
        k > max_i32_accumulation_k || k % (layout == 2 ? 256 : 128) != 0 ||
    ...
    // Only consume a ready, buffer-owned cache for small batches. Rebuilding
    ...
    if (int2 && cached_decode && layout == 2 && act_rows >= 4 && act_rows <= 16 && m >= 4096 && m % 128 == 0 &&
        ...
        if (m >= 16384 && k == 4096) {
            return { act_rows <= 8 ? "kernel_row4_w1a8_m5_tensorops_preexpanded_m8n128_bk512" :
                     ...
                     act_rows <= 8 ? "M5 MPP TensorOps exact A8/I4/I32 M8N128 BK512 cached multi-decode" :
                                     "M5 MPP TensorOps exact A8/I4/I32 M32N128 BK512 cached multi-decode",
                     act_rows <= 8 ? 8 : 32,
                     128,
                     4,
                     true };
        ...
        if (act_rows >= 12) {
            return { "kernel_row4_w1a8_m5_tensorops_preexpanded_m16n64_bk512",
                     "M5 MPP TensorOps exact A8/I4/I32 M16N64 BK512 cached multi-decode",
                     16,
                     64,
                     4,
                     true };
    ...
    // Small Pair2 batches favor LUTs except for wide projections, where a
    // full MPP token tile amortizes the online codebook expansion.
    if (layout == 2 && act_rows < 32) {
        if (act_rows >= 12 && act_rows <= 16 && m >= 16384 && m % 64 == 0 && k <= 4096) {
            return { "kernel_row4_w1a8_m5_tensorops_decode_m16n64_sg4",
        ...
        if (act_rows > 16 && m >= 6144 && m % 128 == 0 && k <= 4096) {
            return { "kernel_row4_w1a8_m5_tensorops_prefill_m32n128",
                     "M5 MPP TensorOps exact A8/I4/I32 M32N128 padded multi-decode",
                     32,
                     128,
                     4,
                     false };
    ...
    if (act_rows >= 256 && act_rows % 256 == 0 && m % 32 == 0) {
        return {
            ...
            "M5 MPP TensorOps exact A8/I4/I32 M256N32 BK128 threadgroup-decode",
            256,
            32,
            8,
            false,
    ...
    if (act_rows >= 128 && act_rows % 128 == 0 && m % 32 == 0) {
        return {
            "kernel_row4_w1a8_m5_tensorops_prefill_m128n32",
            "M5 MPP TensorOps exact A8/I4/I32 M128N32 BK128 threadgroup-decode",
            128,
            32,
            4,
            false,
    ...
    if (act_rows >= 64 && act_rows % 64 == 0 && m % 64 == 0) {
        return {
            "kernel_row4_w1a8_m5_tensorops_prefill_m64n64",
            "M5 MPP TensorOps exact A8/I4/I32 M64N64 BK128 threadgroup-decode",
            64,
            64,
            4,
            false,
    ...
    if (act_rows >= 32 && act_rows % 32 == 0 && m % 128 == 0) {
        return {
            "kernel_row4_w1a8_m5_tensorops_prefill_m32n128",
            "M5 MPP TensorOps exact A8/I4/I32 M32N128 BK128 threadgroup-decode",
            32,
            128,
            4,
            false,
...
static int ggml_metal_w8_m5_row_tile(ggml_metal_device_t dev, const ggml_tensor * op) {
    const int64_t rows = ggml_nrows(op->src[0]);
    const int32_t k    = ggml_get_op_params_i32(op, 2);
    // B2/B3 showed no reliable full-model gain; retain their independent-row path.
    return op->op == GGML_OP_W8A8_LINEAR && ggml_metal_device_get_props(dev)->has_mpp_tensorops && rows >= 4 &&
                   rows <= 32 && ggml_get_op_params_i32(op, 1) % 128 == 0 && k > 0 && k % 128 == 0 &&
                   k <= std::numeric_limits<int32_t>::max() / (128 * 127) ?
               (rows <= 8  ? 8 :
                rows <= 16 ? 16 :
                             32) :
               0;
...
size_t ggml_metal_op_row_quant_linear_extra_act_q(ggml_metal_device_t dev, const ggml_tensor * op) {
    ...
    const ggml_metal_row4_m5_tensorops_config m5_tensorops =
        op->op == GGML_OP_ROW4_LINEAR && act_rows <= (size_t) std::numeric_limits<int32_t>::max() &&
            ...
            ggml_metal_row4_m5_tensorops_select(ggml_metal_device_get_props(dev)->has_mpp_tensorops, false, layout,
                                                ...
                                                ggml_metal_device_get_props(dev)->row4_m5_int2_decode) :
    ...
    const size_t staged_rows = m5_tensorops.pipeline_name ?
                               ...
                               w8_row_tile ? GGML_PAD(act_rows, (size_t) w8_row_tile) :
                                             act_rows;
...
static ggml_tensor * ggml_metal_row4_decode_qat_residual_add(ggml_metal_op_t ctx, int idx, ggml_tensor * op) {
    // Keep the fused epilogue on the two production K shapes that were
    // measured and validated.  In particular, K256 test tensors deliberately
    // retain the standalone ROW4 + COMPLEX_ADD fallback.
...
static int ggml_metal_op_row_quant_linear_impl(ggml_metal_op_t      ctx,
    ...
    const ggml_metal_row4_m5_tensorops_config row4_m5_tensorops = ggml_metal_row4_m5_tensorops_select(
        op->op == GGML_OP_ROW4_LINEAR && !row4_independent_rows &&
            ggml_metal_device_get_props(ctx->dev)->has_mpp_tensorops,
        qat_residual_add != nullptr && act_rows == 1, layout, act_rows, m, k,
        ggml_metal_device_get_props(ctx->dev)->row4_m5_int2,
        ctx->use_row4_cache && ggml_metal_device_get_props(ctx->dev)->row4_m5_int2_decode);
    ...
    const bool    row4_pair2_independent_rows = op->op == GGML_OP_ROW4_LINEAR && layout == 2 && act_rows >= 2 &&
                                                ...
                                                k <= row4_pair2_f32_exact_k_max && !row4_m5_tensorops_prefill;
    const int     w8_row_tile                 = ggml_metal_w8_m5_row_tile(ctx->dev, op);
    const int32_t staged_rows =
        ...
        w8_row_tile ? GGML_PAD(act_rows, w8_row_tile) :
                      act_rows;
    ...
    const size_t         op_allocation = ggml_nbytes(op) + ggml_metal_op_row_quant_linear_extra_act_q(ctx->dev, op);
    ggml_metal_buffer_id act_q         = ggml_metal_get_buffer_id_for_size(op, op_allocation);
    GGML_ASSERT(act_q.metal != nullptr);
    ggml_metal_buffer_id op_buffer = act_q;
    ...
    const bool      row4_v2      = row4 && layout == 2;
    const bool                      pair2_decode = row4_v2 && (path == PATH_DECODE || row4_pair2_independent_rows);
    const ggml_metal_device_props * props_dev             = ggml_metal_device_get_props(ctx->dev);
    const bool pair2_shared_eligible = row4_v2 && !row4_m5_tensorops_prefill && props_dev->has_mpp_tensorops &&
                                       act_rows >= 4 && act_rows < 32 && m % 32 == 0 && k % 256 == 0 &&
                                       k <= row4_pair2_f32_exact_k_max &&
                                       (act_rows >= 8 || m >= 16384 || (k > 4096 && act_rows >= 4));
    const int  pair2_shared_rows     = !pair2_shared_eligible                                                   ? 0 :
                                       act_rows >= 4 && (size_t) k * 4 <= props_dev->max_theadgroup_memory_size ? 4 :
                                       (size_t) k * 2 <= props_dev->max_theadgroup_memory_size                  ? 2 :
                                                                                                                  0;
    const size_t                    pair2_decode_bytes    = (size_t) k * sizeof(int8_t);
    const bool                      pair2_decode_tgmem_ok = !pair2_decode || (k <= row4_pair2_f32_exact_k_max &&
                                                         pair2_decode_bytes <= props_dev->max_theadgroup_memory_size);
    ...
    const bool row4_prefill_m64n32_native_layout_dualw_direct_act = row4_direct_act && !pair2_decode;
    ...
    const char * pipeline_name =
        ...
        qat_residual_add          ? "kernel_row4_w1a8_decode_o16_o4_staged_act_qat_residual" :
        row4_m5_tensorops_prefill ? row4_m5_tensorops.pipeline_name :
        ...
        w8_row_tile == 8                        ? "kernel_row8_w8a8_m5_direct_m8n16" :
        w8_row_tile == 16                       ? "kernel_row8_w8a8_m5_direct_m16n16" :
        w8_row_tile == 32                       ? "kernel_row8_w8a8_m5_direct_m32n16" :
    ...
    const char * row4_path_detail =
        pair2_shared_rows == 4 ?
            "M5 exact F16 packet8/F32 LUT16 O32 B4 shared-weight" :
        pair2_shared_rows == 2 ?
            "M5 exact F16 packet8/F32 LUT16 O32 B2 shared-weight" :
        row4_m5_tensorops_prefill ?
        ...
        row4_decode_o16_o4_staged_act ?
            "A8/I32 O16 one-O4-per-SIMDgroup staged-act four-bank load interleave" :
        ...
        row4_prefill_m64n32_native_layout_dualw ?
            ...
            "half MMA/F32 exact accumulate M16N16 generic";
    ...
    if (log_shape) {
        ...
        if (pair2_shared_rows) {
            GGML_LOG_INFO(
                ...
                "pipeline=lut16-shared-rows split_K=%d threads=%d (%s, BF16 boundary)\n",
                suffix, act_rows, m, k, pair2_shared_nth / 128, pair2_shared_nth, row4_path_detail);
        } else if (row4_v2 && pair2_decode) {
            GGML_LOG_INFO(
            ...
            const char * w8_path_detail = w8_row_tile == 8  ? "M5 MPP TensorOps exact A8/I8/I32 M8N16 direct-device" :
                                          ...
                                          w8_row_tile == 32 ? "M5 MPP TensorOps exact A8/I8/I32 M32N16 direct-device" :
    ...
    if (w8_row_tile) {
        ...
        GGML_ASSERT(nth <= ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
        ggml_metal_encoder_dispatch_threadgroups(enc, m / output_tile, staged_rows / w8_row_tile, 1, nth, 1, 1);
        ...
        const int     nth               = row4_m5_tensorops.n_simdgroups * 32;
        GGML_ASSERT(nth <= ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
    ...
    if (w8_row_tile) {
        ...
        // Dispatched above with its dedicated TensorOps geometry.
    } else if (pair2_shared_rows) {
        const size_t stage_bytes = (size_t) pair2_shared_rows * (size_t) k;
        GGML_ASSERT(act_q.offs % 16 == 0);
        GGML_ASSERT(pair2_shared_nth <= ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));
        GGML_ASSERT(stage_bytes + ggml_metal_pipeline_get_smem(pipeline) <= props_dev->max_theadgroup_memory_size);
        ggml_metal_encoder_set_threadgroup_memory_size(enc, stage_bytes, 0);
        ggml_metal_encoder_dispatch_threadgroups(enc, m / 32, (act_rows + pair2_shared_rows - 1) / pair2_shared_rows, 1,
                                                 pair2_shared_nth, 1, 1);
    } else if (row4_prefill_m64n32_native_layout_dualw_direct_act) {
    ...
    } else if (path == PATH_PREFILL && !pair2_decode) {
        ...
        const size_t scratch_bytes = ggml_metal_pipeline_get_smem(pipeline);
        ...
        ggml_metal_encoder_dispatch_threadgroups(
            enc, m / rows_per_tg, row4_independent_rows || row4_pair2_independent_rows ? act_rows : 1, 1, nth, 1, 1);
...
static int ggml_metal_op_row4_gate_up_swiglu_down(ggml_metal_op_t ctx,
    ...
    const int32_t       layout = ggml_get_op_params_i32(gate_up, 0);
...
int ggml_metal_op_row_quant_linear(ggml_metal_op_t ctx, int idx) {
    ...
    if (ctx->use_fusion && op->op == GGML_OP_ROW4_LINEAR && idx + 5 < ctx->idx_end) {
        ...
        const size_t gate_up_allocation = ggml_nbytes(op) + ggml_metal_op_row_quant_linear_extra_act_q(ctx->dev, op);
        ...
        const bool can_fuse = exact_chain && exact_uses && exact_shapes && scratch_isolated &&
        ...
        if (prefer_m5_tensorops && can_fuse && ctx->debug_fusion > 1) {
            GGML_LOG_DEBUG("%s: bypass Row4 gate-up producer fusion for M5 MPP TensorOps\n", __func__);
        }
        if (!prefer_m5_tensorops && can_fuse) {
...
static bool ggml_metal_flash3_mask_bounds_shape(const ggml_tensor * op) {
    return ggml_flash_attn_ext_get_fairy2i_flash3(op) && op->src[1]->type == GGML_TYPE_BF16 &&
           op->src[2]->type == GGML_TYPE_BF16 && op->src[0]->ne[0] == 128 && op->src[2]->ne[0] == 128 &&
           op->src[0]->ne[1] >= 512 && op->src[1]->ne[1] >= 512 && op->src[3] && op->src[3]->type == GGML_TYPE_F16 &&
           op->src[3]->ne[2] == 1 && !op->src[4];
}
...
size_t ggml_metal_op_flash_attn_ext_extra_tmp(const ggml_tensor * op) {
    ...
    if (!ggml_metal_op_flash_attn_ext_use_vec(op)) {
        return ggml_metal_flash3_mask_bounds_shape(op) ?
                   GGML_PAD(ggml_nbytes(op), 4) - ggml_nbytes(op) +
                       (size_t) GGML_PAD(op->src[0]->ne[1], 8) / 8 * op->src[0]->ne[3] * sizeof(int32_t) :
                   0;
    }
...
static ggml_tensor * ggml_metal_flash3_row4_output(ggml_metal_op_t ctx, int idx) {
    const ggml_tensor * op = ggml_graph_node(ctx->gf, idx);
    if (!ctx->use_fusion || !ggml_metal_device_get_props(ctx->dev)->has_mpp_tensorops ||
        !ggml_flash_attn_ext_get_fairy2i_flash3(op) || idx + 2 >= ctx->idx_end || op->src[1]->type != GGML_TYPE_BF16 ||
        op->src[2]->type != GGML_TYPE_BF16 || op->ne[0] != 128 || op->ne[1] != 32 || op->ne[2] < 32 || op->ne[2] % 32 ||
        op->ne[3] != 1 || !ggml_is_contiguous(op) || !ggml_metal_node_has_n_raw_uses(ctx->gf, idx, 1)) {
        return nullptr;
    }
    const ggml_tensor * view   = ggml_graph_node(ctx->gf, idx + 1);
    ggml_tensor *       linear = ggml_graph_node(ctx->gf, idx + 2);
    if (view->op != GGML_OP_RESHAPE || view->view_src != op || view->view_offs ||
        !ggml_metal_node_has_n_raw_uses(ctx->gf, idx + 1, 1) || !ggml_is_contiguous(view) ||
        linear->op != GGML_OP_ROW4_LINEAR || linear->src[0] != view || ggml_get_op_params_i32(linear, 2) != 4096 ||
        ggml_get_op_params_i32(linear, 1) != 4096 || !ggml_metal_device_supports_op(ctx->dev, linear)) {
        return nullptr;
    }
    ggml_metal_buffer_id scratch = ggml_metal_get_buffer_id_for_size(
        linear, ggml_nbytes(linear) + ggml_metal_op_row_quant_linear_extra_act_q(ctx->dev, linear));
    scratch.offs += GGML_PAD(ggml_nbytes(linear), 64);
    const size_t quant_bytes = (size_t) op->ne[2] * (4096 + sizeof(float));
    if (ggml_metal_row4_range_overlaps_tensor((uintptr_t) linear->data + GGML_PAD(ggml_nbytes(linear), 64), quant_bytes,
                                              op) ||
        ggml_metal_buffer_ranges_overlap(ggml_metal_get_buffer_id(op), ggml_nbytes(op), scratch, quant_bytes)) {
        return nullptr;
    }
    return linear;
}
...
int ggml_metal_op_flash_attn_ext(ggml_metal_op_t ctx, int idx) {
    ...
    const bool                     turbo_chain_fused = ggml_metal_get_turbo4_fattn_fusion(ctx, idx, &fusion);
    const bool                     turbo_fused       = turbo_chain_fused || ggml_flash_attn_ext_get_turbo4_fused(op);
    if (turbo_fused && !turbo_chain_fused) {
        fusion.q_src = op->src[0];
        fusion.dst   = op;
    }
...
#define FATTN_SMEM(nsg)                                                                                              \
    (GGML_PAD(                                                                                                       \
        (size_t(nqptg) * (ne00 + 2 * GGML_PAD(ne20, 64) + 2 * (2 * ncpsg)) + size_t(is_q) * 16 * 32 * size_t(nsg)) * \
            (sizeof(float) / 2),                                                                                     \
        16))
        ...
        ggml_tensor * row4_output = ggml_metal_flash3_row4_output(ctx, idx);
        ggml_tensor * row4_residual =
            row4_output ? ggml_metal_row4_prefill_qat_residual_add(ctx, idx + 2, row4_output) : nullptr;
        if (row4_output && (!ggml_metal_op_concurrency_check(ctx, row4_output) ||
                            (row4_residual && !ggml_metal_op_concurrency_check(ctx, row4_residual)))) {
            ggml_metal_op_concurrency_reset(ctx);
        }
        ...
        const bool           mask_bounds = ctx->use_fusion && props_dev->has_mpp_tensorops &&
                                           ggml_metal_flash3_mask_bounds_shape(op) && !has_bias && !has_scap;
        ggml_metal_buffer_id output =
            mask_bounds ?
                ggml_metal_get_buffer_id_for_size(op, ggml_nbytes(op) + ggml_metal_op_flash_attn_ext_extra_tmp(op)) :
                ggml_metal_get_buffer_id(op);
        if (mask_bounds) {
            args.mask_bounds_offset     = GGML_PAD(ggml_nbytes(op), 4);
            ggml_metal_buffer_id bounds = output;
            bounds.offs += args.mask_bounds_offset;
            const char *          name    = "kernel_fairy2i_flash3_mask_bounds";
            ggml_metal_pipeline_t prepass = ggml_metal_library_get_pipeline(lib, name);
            if (!prepass) {
                prepass = ggml_metal_library_compile_pipeline(lib, name, name, nullptr);
            }
            ggml_metal_encoder_set_pipeline(enc, prepass);
            ggml_metal_encoder_set_bytes(enc, &args, sizeof(args), 0);
            ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[3]), 1);
            ggml_metal_encoder_set_buffer(enc, bounds, 2);
            ggml_metal_encoder_dispatch_threadgroups(enc, (ne01 + 7) / 8, ne03, 1, 256, 1, 1);
            ggml_metal_encoder_memory_barrier(enc);
            if (ctx->debug_fusion > 1) {
                GGML_LOG_DEBUG("FAIRY2I Flash3 mask bounds: queries=%lld kv=%lld\n", (long long) ne01,
                               (long long) ne11);
            }
        }

        args.packed_output = row4_output != nullptr;
        ...
        ggml_metal_encoder_set_buffer(enc, output, 6);
        ...
        if (row4_output) {
            ggml_metal_encoder_memory_barrier(enc);
            ggml_metal_op_row_quant_linear_impl(ctx, row4_output, ggml_metal_get_buffer_id(op), true, row4_residual);
            if (ctx->debug_fusion > 1) {
                GGML_LOG_DEBUG("FAIRY2I Flash3 packed BF16 -> Row4 A8: queries=%lld K=4096\n", (long long) ne01);
            }
            return row4_residual ? 4 : 3;
        }
#undef FATTN_SMEM
    } else {
        // half4x4 kernel
        ...
        //
#define FATTN_SMEM(nsg)                                                                  \
    (GGML_PAD((size_t(nqptg) * (GGML_PAD(ne00, 128) + size_t(4) * ncpsg * size_t(nsg)) + \
               size_t(2) * GGML_PAD(ne20, 128) * size_t(nsg)) *                          \
                  (sizeof(float) / 2),                                                   \
              16))
        ...
        GGML_ASSERT(nsg <= std::numeric_limits<int32_t>::max());
        ggml_metal_pipeline_t pipeline = ggml_metal_library_get_pipeline_flash_attn_ext_vec(
            lib, op, has_mask, has_sinks, has_bias, has_scap, (int32_t) nsg, nwg, turbo_fused, turbo_gqa4);

        GGML_ASSERT((turbo_gqa4 ? 4 : nsg) * 32 <= ggml_metal_pipeline_max_theads_per_threadgroup(pipeline));

        ggml_metal_encoder_set_pipeline(enc, pipeline);
        ggml_metal_encoder_set_bytes   (enc, &args, sizeof(args), 0);
        ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(turbo_fused ? fusion.q_src : op->src[0]), 1);
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[1]), 2);
        ggml_metal_encoder_set_buffer  (enc, ggml_metal_get_buffer_id(op->src[2]), 3);
        if (op->src[3]) {
            ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(op->src[3]), 4);
        ...
        if (nwg == 1) {
            ...
            ggml_metal_encoder_set_buffer(enc, ggml_metal_get_buffer_id(turbo_fused ? fusion.dst : op), 6);
            ...
            ggml_metal_buffer_id bid_dst = ggml_metal_get_buffer_id(turbo_fused ? fusion.dst : op);
            ...
            ggml_metal_encoder_set_threadgroup_memory_size(enc, smem, 0);
            GGML_ASSERT(ne01 <= std::numeric_limits<int>::max() && ne02 <= std::numeric_limits<int>::max() &&
                        ne03 * nwg <= std::numeric_limits<int>::max() && ne12 <= std::numeric_limits<int>::max() &&
                        nsg <= std::numeric_limits<int>::max());
            ggml_metal_encoder_dispatch_threadgroups(enc, (int) ((ne01 + nqptg - 1) / nqptg),
                                                     (int) (turbo_gqa4 ? ne12 : ne02), (int) (ne03 * nwg), 32,
                                                     turbo_gqa4 ? 4 : (int) nsg, 1);
