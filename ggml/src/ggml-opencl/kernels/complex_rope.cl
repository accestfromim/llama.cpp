...
kernel void kernel_complex_rope(global void *  src0,
                               ulong          offset0,
                               global int *   src1,
                               ulong          offset1,
                               global float * src2,
    ...
    // Keep the same assumptions as CPU complex rope path: fixed base=10000,
    // no YaRN scaling and no frequency factors. The frequency denominator is
    // the full complex dimension even when n_dims is partial.
    ...
    for (int i0 = get_local_id(0); i0 < n_dims; i0 += get_local_size(0)) {
        ...
        const float x0     = complex_bf16_to_f32((ushort) (packed & 0xFFFFu));
        const float x1     = complex_bf16_to_f32((ushort) (packed >> 16));
