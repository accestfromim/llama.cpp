...
static inline uint ifairy64_pack4_s8(int x0, int x1, int x2, int x3) {
...
static inline int ifairy64_sum4_s8(uint packed) {
...
static inline int ifairy64_code_real(uint code) {
    const int sign = (int) ((code & 1U) << 1) - 1;
    const int imag = (int) (code >> 1);
    return sign * (1 - imag);
}

static inline int ifairy64_code_imag(uint code) {
    const int sign = (int) ((code & 1U) << 1) - 1;
    const int imag = (int) (code >> 1);
    return sign * imag;
}

static inline uint ifairy64_pack4_coeff_real_u8(uint packed) {
    return (uint) (uchar) (128 + ifairy64_code_real(packed & 3U)) |
          ((uint) (uchar) (128 + ifairy64_code_real((packed >> 2) & 3U)) << 8) |
          ((uint) (uchar) (128 + ifairy64_code_real((packed >> 4) & 3U)) << 16) |
          ((uint) (uchar) (128 + ifairy64_code_real((packed >> 6) & 3U)) << 24);
}

static inline uint ifairy64_pack4_coeff_imag_u8(uint packed) {
    return (uint) (uchar) (128 + ifairy64_code_imag(packed & 3U)) |
          ((uint) (uchar) (128 + ifairy64_code_imag((packed >> 2) & 3U)) << 8) |
          ((uint) (uchar) (128 + ifairy64_code_imag((packed >> 4) & 3U)) << 16) |
          ((uint) (uchar) (128 + ifairy64_code_imag((packed >> 6) & 3U)) << 24);
}

static inline int ifairy64_dot4_s8u8_qcom(uint x_s8, uint coeff_u8, int correction) {
    return qcom_dot8_acc(x_s8, coeff_u8, correction);
}

static inline int4 ifairy64_dot8_sums_packed(
        uint packed,
        uint xr_pack,
        uint xi_pack,
        int  xr_correction,
    ...
    const uint cr_pack = ifairy64_pack4_coeff_real_u8(packed);
    const uint ci_pack = ifairy64_pack4_coeff_imag_u8(packed);
    return (int4)(
        ifairy64_dot4_s8u8_qcom(xr_pack, cr_pack, xr_correction),
        ifairy64_dot4_s8u8_qcom(xi_pack, cr_pack, xi_correction),
        ifairy64_dot4_s8u8_qcom(xr_pack, ci_pack, xr_correction),
        ifairy64_dot4_s8u8_qcom(xi_pack, ci_pack, xi_correction));
}

/**
 * Fused iFairy64 wide-linear W2 using Qualcomm dot8 for the four 2-bit lanes
 * packed in each weight byte. The activation q16 staging and final complex
 * accumulation semantics match kernel_ifairy64_wide_linear_w2_f32_q16.
 */
__attribute__((reqd_work_group_size(64, 1, 1)))
kernel void kernel_ifairy64_wide_linear_w2_f32_q16_dot8(
        global uchar * u0_q,
        global half  * u0_d,
        global uchar * u1_q,
        global half  * u1_d,
...
#pragma unroll
    ...
    for (int wb_base = 0; wb_base < nb64; wb_base += 4) {
        ...
        if (wb < nb64) {
            ...
            const uint xr_pack = ifairy64_pack4_s8(
                (int) act_q[q_base + lane + 0],
                (int) act_q[q_base + lane + 16],
                (int) act_q[q_base + lane + 32],
                (int) act_q[q_base + lane + 48]);
            const uint xi_pack = ifairy64_pack4_s8(
                (int) act_q[q_base + QK_IFAIRY64_Q16 + lane + 0],
                (int) act_q[q_base + QK_IFAIRY64_Q16 + lane + 16],
                (int) act_q[q_base + QK_IFAIRY64_Q16 + lane + 32],
                (int) act_q[q_base + QK_IFAIRY64_Q16 + lane + 48]);
            const int xr_sum = ifairy64_sum4_s8(xr_pack);
            const int xi_sum = ifairy64_sum4_s8(xi_pack);
...
#pragma unroll
            for (int tr = 0; tr < IFAIRY64_WIDE_TILE_M; ++tr) {
                ...
                const int4 u0 = ifairy64_dot8_sums_packed(packed_u0, xr_pack, xi_pack, xr_correction, xi_correction);
                const int4 u1 = ifairy64_dot8_sums_packed(packed_u1, xr_pack, xi_pack, xr_correction, xi_correction);
                const int4 w0 = ifairy64_dot8_sums_packed(packed_w0, xr_pack, xi_pack, xr_correction, xi_correction);
                const int4 w1 = ifairy64_dot8_sums_packed(packed_w1, xr_pack, xi_pack, xr_correction, xi_correction);
