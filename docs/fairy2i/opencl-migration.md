...
GGML_OPENCL_FAIRY2I_WIDE_LINEAR_W2_IMPL=q16|q16dot8
```
...
| `GGML_FAIRY2I_OPENCL=ON`, `GGML_OPENCL_USE_ADRENO_KERNELS=ON` | above plus `fairy2i_tile64_dot8` | `GGML_OPENCL_FAIRY2I=1`, optional `GGML_OPENCL_FAIRY2I_WIDE_LINEAR_W2_IMPL=q16dot8` |
...
complex_split
fairy2i_tile64
fairy2i_tile64_dot8 (Adreno dot8 W2 path only)
```
...
`GGML_OP_FAIRY2I_WIDE_LINEAR_W2` uses the same q16_64 activation staging, then
dispatches a second fused W2 kernel over `U.s0/U.s1/W.s0/W.s1`. The default
implementation is `q16`. On Adreno devices exposing `cl_qcom_dot_product8`,
`GGML_OPENCL_FAIRY2I_WIDE_LINEAR_W2_IMPL=q16dot8` selects the dot8 variant.
...
- W2 dot8 is not selected: check the build uses
  `GGML_OPENCL_USE_ADRENO_KERNELS=ON`, runtime uses
  `GGML_OPENCL_FAIRY2I_WIDE_LINEAR_W2_IMPL=q16dot8`, and the device reports
