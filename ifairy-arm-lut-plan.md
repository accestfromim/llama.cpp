...
- **执行记录（2025-02-05）**：完成权重解码瘦身、激活预打包与 qgemm 偶/奇融合+解码-计算融合，构建 `cmake --build build --config Release -j $(nproc)` 通过，`llama-bench`（同上参数）输出 pp512 45.83 tok/s、tg128 29.68 tok/s（Apple M4, 4 线程, Metal+BLAS；Metal/线程调度存在波动，需补 CPU-only A/B）。
