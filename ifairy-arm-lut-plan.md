...
- **执行步骤**：
  ...
  3) 利用权重取值 ±1/±i 的对称性，统一 wr/wi 查表和激活 pack 布局，减少访存通道数。
  4) 保留形状选择/回退逻辑，完成构建 + `./build/bin/llama-bench -m models/Fairy-plus-minus-i-700M/ifairy.gguf --threads 4 --n-prompt 512 --n-gen 128 -ngl 0` 基准记录 tok/s。
- **验收**：功能正确（与参考内核结果一致），LUT 路径在目标形状下优于现有 `vec_dot`，文档同步记录方案与基准。
- **执行记录（2025-02-05）**：完成权重解码瘦身、激活预打包与 qgemm 偶/奇融合，构建 `cmake --build build --config Release -j $(nproc)` 通过，`llama-bench`（同上参数）输出 pp512 51.41 tok/s、tg128 27.87 tok/s（Apple M4, 4 线程, Metal+BLAS）。
