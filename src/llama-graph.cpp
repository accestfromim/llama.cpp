...
static ggml_tensor * llm_fairy2i_bf16_roundtrip(ggml_context * ctx, ggml_tensor * tensor) {
...
ggml_tensor * llm_graph_context::build_attn_mha(ggml_tensor * q,
    ...
    if (fairy2i_exact && !cparams.flash_attn) {
        ...
        cur = ggml_fairy2i_attn_exact_cpu(ctx0, q, k, v, kq_mask, kq_scale);
        cb(cur, "fairy2i_attn_exact_cpu", il);
    ...
    if (cparams.flash_attn &&
        ...
        if (k->type == GGML_TYPE_F32) {
            k = ggml_cast(ctx0, k, fairy2i_exact ? GGML_TYPE_BF16 : GGML_TYPE_F16);
        }
        ...
        ggml_flash_attn_ext_set_turbo4_fused(cur, turbo4_fused);
        ...
        if (fairy2i_exact) {
            ...
            cb(kq, "kq_scaled_exact", il);
            kq = llm_fairy2i_bf16_roundtrip(ctx0, kq);
            cb(kq, "kq_scaled_bf16_exact", il);

            GGML_ASSERT(kq_mask && kq_mask->type == GGML_TYPE_F32);
            GGML_ASSERT(kq_mask->ne[0] == kq->ne[0] && kq_mask->ne[1] >= kq->ne[1] && kq_mask->ne[2] == 1 &&
                        kq_mask->ne[3] == kq->ne[3]);
            ggml_tensor * kq_mask_view = ggml_view_4d(ctx0, kq_mask, kq->ne[0], kq->ne[1], 1, kq->ne[3], kq_mask->nb[1],
                                                      kq_mask->nb[2], kq_mask->nb[3], 0);
            kq                         = ggml_add(ctx0, kq, kq_mask_view);
            cb(kq, "kq_masked_exact", il);
            kq = llm_fairy2i_bf16_roundtrip(ctx0, kq);
            cb(kq, "kq_masked_bf16_exact", il);

            kq = ggml_soft_max_ext(ctx0, kq, nullptr, 1.0f, 0.0f);
            cb(kq, "kq_soft_max_exact", il);
            kq = llm_fairy2i_bf16_roundtrip(ctx0, kq);
            cb(kq, "kq_soft_max_bf16_exact", il);
        } else {
        ...
        if (fairy2i_exact) {
            ...
            kqv = llm_fairy2i_bf16_roundtrip(ctx0, kqv);
            cb(kqv, "kqv_bf16_exact", il);
        }
