...
static bool llama_fairy2i_output_neon_enabled() {
    const char * env = getenv("LLAMA_FAIRY2I_OUTPUT_NEON");
    return env != nullptr && strcmp(env, "0") != 0;
}
...
void llama_model::load_hparams(llama_model_loader & ml) {
    ...
    if (hparams.n_head() > 0) {
        ...
        if (arch == LLM_ARCH_IFAIRY || arch == LLM_ARCH_FAIRY2I) {
        ...
        case LLM_ARCH_FAIRY2I:
            {
                ...
                if (quant_variant == "legacy") {
                    ...
                    uint32_t residual_steps = 0;
                    ml.get_key("fairy2i.quant.residual_steps", residual_steps);
                    if (residual_steps != 2) {
                        throw std::runtime_error(
                            format("invalid FAIRY2I tile64_v2 residual steps: %u, expected 2", residual_steps));
                    }
                ...
                } else if (quant_variant == "tile64_v2_w1_learned_scale") {
                    ...
                    if (residual_steps != 1) {
                        throw std::runtime_error(
                            format("invalid FAIRY2I tile64_v2_w1_learned_scale residual steps: %u, expected 1",
                                   residual_steps));
                    }
                    ...
                    if (tile_size != 64) {
                        throw std::runtime_error(
                            format("invalid FAIRY2I tile64_v2_w1_learned_scale tile size: %u, expected 64", tile_size));
                    }

                    std::string scale_source;
                    ml.get_key("fairy2i.quant.scale_source", scale_source);
                    if (scale_source != "learned") {
                        throw std::runtime_error(
                            format("invalid FAIRY2I tile64_v2_w1_learned_scale scale source: %s, expected learned",
                                   scale_source.c_str()));
                    }
                } else {
                    throw std::runtime_error(
                ...
                if (weight_layout == "tile64_v2") {
                    ...
                    if ((fairy2i_schema_version != 2 && fairy2i_schema_version != 3) || alignment < 64 ||
                        m_block != 64 || k_block != 64 || m_subtile != 16) {
                ...
                if (fairy2i_uses_exact_numeric_profile()) {
                    ...
                    if (llama_fairy2i_output_neon_enabled()) {
                        throw std::runtime_error(
                            "FAIRY2I script_f32reduce_bf16scale_v1 forbids LLAMA_FAIRY2I_OUTPUT_NEON; "
                            "the exact profile requires the W2 bundle output");
                    }
                }
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    const bool fairy2i_output_neon = arch == LLM_ARCH_FAIRY2I && llama_fairy2i_output_neon_enabled();

    const bool use_mmap_buffer = true;

    LLAMA_LOG_INFO("%s: loading model tensors, this can take a while... (mmap = %s)\n", __func__, ml.use_mmap ? "true" : "false");
    if (fairy2i_output_neon) {
        LLAMA_LOG_INFO("%s: LLAMA_FAIRY2I_OUTPUT_NEON=1, dense output matmul will use the CPU backend when available\n",
                       __func__);
    }
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            if (dense_fairy2i_output) {
                LLAMA_LOG_INFO("%s: placing dense output tensor %s on CPU backend for LLAMA_FAIRY2I_OUTPUT_NEON\n",
            ...
            if (dense_fairy2i_output) {
                ...
                if (!weight_buft_supported(hparams, t_meta, op, buft, cpu_dev)) {
                    throw std::runtime_error(
                        format("CPU backend does not support dense Fairy2i output tensor %s", tn.str().c_str()));
                }
            }
            ...
            case LLM_ARCH_FAIRY2I:
                {
                    ...
                    output_norm = create_tensor(tn(LLM_TENSOR_OUTPUT_NORM), { n_embd * 2 }, 0);
                    auto require_exact_f32_carrier = [&](const ggml_tensor * tensor, const char * role) {
                        if (fairy2i_uses_exact_numeric_profile() && tensor->type != GGML_TYPE_F32) {
                            throw std::runtime_error(format(
                                "FAIRY2I script_f32reduce_bf16scale_v1 requires an F32 carrier tensor for %s (%s); "
                                "got %s",
                                role, tensor->name, ggml_type_name(tensor->type)));
                        }
                    };
                    require_exact_f32_carrier(tok_embd, "checkpoint BF16 token embedding");
                    require_exact_f32_carrier(output_norm, "checkpoint BF16 output norm");
                    ...
                    if (fairy2i_uses_exact_numeric_profile() && output) {
                        throw std::runtime_error(
                    ...
                    auto create_fairy2i_stage = [&](ggml_tensor * & stage, llm_tensor tensor, const char * suffix, int bid,
                        ...
                        const auto expected_type = fairy2i_is_tile64 ? GGML_TYPE_FAIRY2I_TILE64_V2 : GGML_TYPE_IFAIRY;
                    ...
                    auto reject_fairy2i_stage = [&](llm_tensor tensor, const char * suffix, int bid) {
                        const auto stage_tn = bid >= 0 ? tn(tensor, suffix, bid) : tn(tensor, suffix);
                        if (ml.get_tensor_meta(stage_tn.str().c_str())) {
                            throw std::runtime_error(format(
                                "invalid FAIRY2I W1 tensor set: %s is not allowed for tile64_v2_w1_learned_scale",
                                stage_tn.str().c_str()));
                        }
                    };
                    ...
                    auto create_fairy2i_linear = [&](llama_widely_linear_ifairy & linear, llm_tensor tensor, int bid,
                        ...
                        if (fairy2i_is_bundle) {
                            ...
                            const ggml_type expected_scales_type =
                                fairy2i_uses_exact_numeric_profile() ? GGML_TYPE_BF16 : GGML_TYPE_F16;
                            if (codes_meta->type != GGML_TYPE_FAIRY2I_BUNDLE_CODES ||
                                ...
                                throw std::runtime_error(
                                    format("invalid FAIRY2I bundle_v1 types for %s under schema v%u/%s: "
                                           ...
                                           (bid >= 0 ? tn(tensor, bid).str() : tn(tensor).str()).c_str(),
                                           fairy2i_schema_version,
                                           fairy2i_uses_exact_numeric_profile() ? "script_f32reduce_bf16scale_v1" :
                                                                                  "legacy_f16_v1",
                        ...
                        if (fairy2i_is_w1) {
                            reject_fairy2i_stage(tensor, "U.s1", bid);
                            reject_fairy2i_stage(tensor, "W.s1", bid);
                        } else {
                    ...
                    const bool has_output_fairy2i     = fairy2i_linear_complete(output_fairy2i);
                    ...
                    if (fairy2i_uses_exact_numeric_profile() && !has_output_fairy2i) {
                        throw std::runtime_error(
                    ...
                    const bool output_neon = llama_fairy2i_output_neon_enabled();
                    ...
                    if (output_neon && !output && has_output_fairy2i) {
                        LLAMA_LOG_WARN(
                            "%s: LLAMA_FAIRY2I_OUTPUT_NEON is set, but dense output tensor is missing; "
                            "falling back to output.{U,W}.s{0,1}\n",
                            __func__);
                    }
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        layer.attn_norm = create_tensor(tn(LLM_TENSOR_ATTN_NORM, i), { n_embd * 2 }, 0);
                        layer.ffn_norm  = create_tensor(tn(LLM_TENSOR_FFN_NORM, i), { n_embd * 2 }, 0);
                        require_exact_f32_carrier(layer.attn_norm, "checkpoint BF16 attention norm");
                        require_exact_f32_carrier(layer.ffn_norm, "checkpoint BF16 FFN norm");
                        if (fairy2i_is_qwen3) {
                        ...
                        const int qkv_bias_flags = fairy2i_uses_exact_numeric_profile() ? 0 : TENSOR_NOT_REQUIRED;
                        ...
                        require_exact_f32_carrier(layer.bq, "checkpoint BF16 Q bias");
                        require_exact_f32_carrier(layer.bk, "checkpoint BF16 K bias");
                        require_exact_f32_carrier(layer.bv, "checkpoint BF16 V bias");
...
void llama_model::print_info() const {
    ...
    if (arch == LLM_ARCH_FAIRY2I) {
        ...
        LLAMA_LOG_INFO("%s: fairy2i numeric  = %s\n", __func__,
                       fairy2i_uses_exact_numeric_profile() ? "script_f32reduce_bf16scale_v1" : "legacy_f16_v1");
    }
...
struct llm_build_fairy2i : public llm_graph_context {
    llm_build_fairy2i(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
        const bool    fairy2i_exact = model.fairy2i_uses_exact_numeric_profile();
        ...
        if (fairy2i_exact && model.fairy2i_attn_layout != LLAMA_FAIRY2I_ATTN_LAYOUT_QWEN2_REAL) {
        ...
        auto build_complex_conj = [&](ggml_tensor * x, int il, const char * name) -> ggml_tensor * {
            ...
            ggml_tensor * x_real = ggml_view_2d(ctx0, x_split, n_half, x_split->ne[1], x_split->nb[1], 0);
            ggml_tensor * x_imag =
                ggml_view_2d(ctx0, x_split, n_half, x_split->ne[1], x_split->nb[1], n_half * x_split->nb[0]);
        ...
        auto build_fairy2i_rms_norm = [&](ggml_tensor * x, ggml_tensor * norm_w, int il,
            ...
            ggml_tensor * x_split = ggml_complex_split(ctx0, x);
            ggml_tensor * x_norm  = fairy2i_exact ?
        ...
        auto build_wide_linear = [&](const llama_widely_linear_ifairy & linear, ggml_tensor * x, ggml_tensor * x_conj,
            ...
            if (linear.bundle_codes || linear.bundle_scales) {
                ...
                if (!target_dev || !ggml_backend_dev_supports_op(target_dev, y)) {
                    if (fairy2i_exact && target_dev_is_metal) {
                        throw std::runtime_error(format(
                            "FAIRY2I script_f32reduce_bf16scale_v1 target device %s does not support the exact W2 "
                            "pipeline; native Metal BF16 and simdgroup matrix support are required, and "
                            "FP16 fallback is forbidden",
                            target_dev ? ggml_backend_dev_name(target_dev) : "none"));
                    }
                    throw std::runtime_error(
            ...
            const bool can_fuse_w1 =
                ...
                can_fuse_tile64 && !has_stage1;
            if (target_dev_is_cpu && can_fuse_w1) {
                ggml_tensor * y = ggml_fairy2i_wide_linear_w1(ctx0, x, linear.U[0], linear.W[0], bias);
                if (llama_fairy2i_should_use_fused_wide_linear(
                        "LLAMA_FAIRY2I_FUSED_WIDE_LINEAR_W1", "GGML_OP_FAIRY2I_WIDE_LINEAR_W1", true, backend_cpu, y)) {
                    cb(y, name, il);
                    return y;
                }
            }

            const bool can_fuse_w2 = can_fuse_tile64 && has_stage1;
            if (target_dev_is_cpu && can_fuse_w2) {
                ggml_tensor * y =
                    ggml_fairy2i_wide_linear_w2(ctx0, x, linear.U[0], linear.U[1], linear.W[0], linear.W[1], bias);
                if (llama_fairy2i_should_use_fused_wide_linear(
                        "LLAMA_FAIRY2I_FUSED_WIDE_LINEAR_W2", "GGML_OP_FAIRY2I_WIDE_LINEAR_W2", true, backend_cpu, y)) {
                    cb(y, name, il);
                    return y;
                }
            }

            if (!target_dev_is_cpu && can_fuse_tile64 && llama_fairy2i_fused_wide_linear_w2_enabled()) {
                ggml_tensor * y =
                    ggml_fairy2i_wide_linear_w2(ctx0, x, linear.U[0], linear.U[1], linear.W[0], linear.W[1], bias);
                cb(y, name, il);
                return y;
        ...
        for (int il = 0; il < n_layer; ++il) {
            ...
            if (use_real_attn) {
                ...
                if (fairy2i_exact && (model.layers[il].attn_q_norm || model.layers[il].attn_k_norm)) {
                    throw std::runtime_error(
            ...
            gate_s = fairy2i_exact ? ggml_fairy2i_silu_exact(ctx0, gate_s) : ggml_silu(ctx0, gate_s);
            if (fairy2i_exact) {
                cb(gate_s, "ffn_gate_silu_bf16_exact", il);
            }

            ggml_tensor * mul_s =
                fairy2i_exact ? ggml_fairy2i_mul_exact(ctx0, gate_s, up_s) : ggml_mul(ctx0, gate_s, up_s);
            if (fairy2i_exact) {
            ...
            if (fairy2i_exact && cvec->tensor_for(il) != nullptr) {
                throw std::runtime_error(
                    "FAIRY2I script_f32reduce_bf16scale_v1 does not support control-vector adapters; "
        ...
        if (fairy2i_exact && has_output_fairy2i_merged) {
            throw std::runtime_error(
        ...
        if (model.fairy2i_uses_exact_numeric_profile() && force_dense_output) {
            throw std::runtime_error(
        ...
        if (has_output_fairy2i_merged && (!force_dense_output || !model.output)) {
            ...
            throw std::runtime_error("FAIRY2I output layer requires dense output tensor or output.{U,W}.s0/s{0,1}");
        }
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_FAIRY2I:
            {
                llm = std::make_unique<llm_build_fairy2i>(*this, params);
            }
            break;
        case LLM_ARCH_T5:
            {
...
llama_rope_type llama_model_rope_type(const llama_model * model) {
    if (model->arch == LLM_ARCH_FAIRY2I && (model->fairy2i_attn_layout == LLAMA_FAIRY2I_ATTN_LAYOUT_QWEN2_REAL ||
                                            model->fairy2i_attn_layout == LLAMA_FAIRY2I_ATTN_LAYOUT_QWEN3_REAL)) {
