...
struct llm_build_fairy2i : public llm_graph_context {
    llm_build_fairy2i(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        ...
        auto build_wide_linear = [&](const llama_widely_linear_ifairy & linear, ggml_tensor * x, ggml_tensor * x_conj,
            ...
            const bool cpu_only = model.devices.empty() || model.params.n_gpu_layers <= 0;
            const bool no_lora  = loras == nullptr || loras->empty();
            const bool can_fuse = llama_fairy2i_fused_wide_linear_enabled() && no_lora && n_tokens == 1 &&
                                  cpu_only &&
                                  linear.U[1] && linear.W[1] &&
                                  linear.U[0]->type == GGML_TYPE_IFAIRY64 && linear.U[1]->type == GGML_TYPE_IFAIRY64 &&
                                  linear.W[0]->type == GGML_TYPE_IFAIRY64 && linear.W[1]->type == GGML_TYPE_IFAIRY64 &&
                                  x->type == GGML_TYPE_F32 && x_conj->type == GGML_TYPE_F32;
            if (can_fuse) {
                ggml_tensor * y = ggml_ifairy_wide_linear(ctx0, linear.U[0], linear.U[1], linear.W[0], linear.W[1],
                                                          x, x_conj);
